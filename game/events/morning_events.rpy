label morning_events():
    ...
    if int(current_time[:2]) in morning and not morning_event_done:
        call fp_bedroom_fp_scene from _call_fp_bedroom_fp_scene_2
        if alarmclock and int(current_time[:2]) < 7 and day_week >= 5:
            fp "Ah, early start on the weekend... Wonder what the rest of the house is up to..."
            menu:
                "Go check on [fmName.yourformal]":
                    pass
            ...
            if (fpmc_r < .35 and day_week <= 4 and overslept and int(current_time[:2]) > 7) or (int(current_time[:2]) > 7 and day_week <= 4):
                ...
                label fm_morningchoice_dom(called=False):
                    if called:
                        ...
                        if fm_dom >= 45:
                            ...
                            $ fm_apologize = True
                            $ addtime(False,10)
                            "[fmName.Yourinformal] walks out of the room"
                ...
                $ breakfast_jump = True
                call fp_bedroom_fp(True) from _call_fp_bedroom_fp_3
            else:
                $ overslept = False
                $ current_time = "06:00"
                if not backpack.has_item(schoolbooks_item):
                        ...
                        if late_oh_shit:
                            label late_morning():
                                if day_week <= 4:
                                    ...
                                    call fp_outside_scene from _call_outside_scene_2
                ...
                if renpy.random.random() > .95: #changed from 6
                    if fs_mad != 1:
                        ...
                        if renpy.random.random() > .5:
                            ...
                            if fs_rel > 15 and fs_aro > 10:
                                ...
                                fp "Cool"
                                $ morning_event_done = True
                                call fp_kitchen(True) from _call_fp_kitchen
                        else:
                            $ morning_event_done = True
                            if renpy.random.random() > .5:
                                $ sun_event = True
                                ...
                                call fp_kitchen(True) from _call_fp_kitchen_1
                    else:
                        # if renpy.random.random() > .5:
                    ...
                    if debug:
                        if renpy.random.random() > .1:
                            ...
                            call fp_kitchen(True) from _call_fp_kitchen_2
                    else:
                        if renpy.random.random() > .5:
                            $ sun_event = True
                            call weekend_sun(True) from _call_weekend_sun_2
                        else:
                            call fp_kitchen(True) from _call_fp_kitchen_3