label fp_aquire_couch():
    ...
    menu:
        ...
        "No":
            ...
            call change_loc(current_location) from _call_change_loc_92
        "Yes":
            $ fp_couch_aquired = True
            $ statschangenotify('fp_money',-250)
            call change_loc(current_location) from _call_change_loc_91
...
label fp_aquire_art(trans=True):
    ...
    menu buyart:
        ...
        "No":
            call change_loc(current_location) from _call_change_loc_99
        "Yes":
            menu:
                "Buy Parking lot wallart":
                    $ wallart['parkinglot'] = True
                    $ active_wallart = 'parkinglot'
                    $ statschangenotify('fp_money',-75)
                    $ update_all_wallart_achievement()
                    call change_loc(current_location) from _call_change_loc_93
                "Buy Ferrari wallart":
                    $ wallart['ferrari'] = True
                    $ active_wallart = 'ferrari'
                    $ statschangenotify('fp_money',-75)
                    $ update_all_wallart_achievement()
                    call change_loc(current_location) from _call_change_loc_94
                "Buy Roadtrip wallart":
                    $ wallart['roadtrip'] = True
                    $ active_wallart = 'roadtrip'
                    $ statschangenotify('fp_money',-75)
                    $ update_all_wallart_achievement()
                    call change_loc(current_location) from _call_change_loc_95
                "Buy Sin City wallart":
                    $ wallart['sincity'] = True
                    $ active_wallart = 'sincity'
                    $ statschangenotify('fp_money',-75)
                    $ update_all_wallart_achievement()
                    call change_loc(current_location) from _call_change_loc_96
                "Buy Peekaboo wallart":
                    $ wallart['peekaboo'] = True
                    $ active_wallart = 'peekaboo'
                    $ statschangenotify('fp_money',-75)
                    $ update_all_wallart_achievement()
                    call change_loc(current_location) from _call_change_loc_97
                "No thanks, not right now":
                    call change_loc(current_location) from _call_change_loc_98